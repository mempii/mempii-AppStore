...
described in detail in the [Compose Build Specification](/reference/compose-file/services/build/) documentation.
...
described in detail in the [Compose Deploy Specification](/reference/compose-file/services/deploy/) documentation.
...
`build` specifies the build configuration for creating a container image from source, as defined in the [Compose Build Specification](/reference/compose-file/services/build/).
...
an integer value using microseconds as unit or a [duration](/reference/compose-file/services/extension/#specifying-durations).
...
When set, `cpus` must be consistent with the `cpus` attribute in the [Deploy Specification](/reference/compose-file/services/deploy/#cpus).
...
> [!NOTE]
>
> Unlike the `CMD` instruction in a Dockerfile, the `command` field doesn't automatically run within the context of the [`SHELL`](/reference/dockerfile/#shell-form) instruction defined in the image. If your `command` relies on shell-specific features, such as environment variable expansion, you need to explicitly run it within a shell. For example:
>
> ```yaml
> command: /bin/sh -c 'echo "hello $$HOSTNAME"'
> ```

The value can also be a list, similar to the [exec-form syntax](/reference/dockerfile/#exec-form)
used by the [Dockerfile](/reference/dockerfile/#exec-form).
...
[`configs` top-level element](/reference/compose-file/services/configs/) in the Compose file.
...
`deploy` specifies the configuration for the deployment and lifecycle of services, as defined [in the Compose Deploy Specification](/reference/compose-file/services/deploy/).

### `develop`



`develop` specifies the development configuration for maintaining a container in sync with source, as defined in the [Development Section](/reference/compose-file/services/develop/).
...
- Unquoted and double-quoted (`"`) values have [Interpolation](/reference/compose-file/services/interpolation/) applied.
...
`gpus` specifies GPU devices to be allocated for container usage. This is equivalent to a [device request](/reference/compose-file/services/deploy/#devices) with
...
For more information on `HEALTHCHECK`, see the [Dockerfile reference](/reference/dockerfile/#healthcheck).
...
`interval`, `timeout`, `start_period`, and `start_interval` are [specified as durations](/reference/compose-file/services/extension/#specifying-durations). Introduced in Docker Compose version [2.20.2](https://github.com/docker/compose/releases/tag/v2.20.2)
...
If you are also using the [Compose Build Specification](/reference/compose-file/services/build/), there are alternative options for controlling the precedence of
...
`mem_limit` configures a limit on the amount of memory a container can allocate, set as a string expressing a [byte value](/reference/compose-file/services/extension/#specifying-byte-values).

When set, `mem_limit` must be consistent with the `limits.memory` attribute in the [Deploy Specification](/reference/compose-file/services/deploy/#memory).

### `mem_reservation`

`mem_reservation` configures a reservation on the amount of memory a container can allocate, set as a string expressing a [byte value](/reference/compose-file/services/extension/#specifying-byte-values).

When set, `mem_reservation` must be consistent with the `reservations.memory` attribute in the [Deploy Specification](/reference/compose-file/services/deploy/#memory).
...
attribute that only has meaning if [`memory`](/reference/compose-file/services/deploy/#memory) is also set. Using swap lets the container write excess
...
`models` defines which AI models the service should use at runtime. Each referenced model must be defined under the [`models` top-level element](/reference/compose-file/services/models/).
...
services:
  some-service:
    networks:
      ...
      - other-network
```
For more information about the `networks` top-level element, see [Networks](/reference/compose-file/services/networks/).
...
The corresponding network configuration in the [top-level networks section](/reference/compose-file/services/networks/) must have an
...
When set, `pids_limit` must be consistent with the `pids` attribute in the [Deploy Specification](/reference/compose-file/services/deploy/#pids).

### `platform`

`platform` defines the target platform the containers for the service run on. It uses the `os[/arch[/variant]]` syntax.

The values of `os`, `arch`, and `variant` must conform to the convention used by the [OCI Image Spec](https://github.com/opencontainers/image-spec/blob/v1.0.2/image-../compose-file.md).
...
- `HOST` is `[IP:](/reference/compose-file/services/port | range)` (optional). If it is not set, it binds to all network interfaces (`0.0.0.0`).
...
- `missing`: Compose pulls the image only if it's not available in the platform cache.
   This is the default option if you are not also using the [Compose Build Specification](/reference/compose-file/services/build/).
...
When both are set, `scale` must be consistent with the `replicas` attribute in the [Deploy Specification](/reference/compose-file/services/deploy/#replicas).
...
[`secrets` top-level section](/reference/compose-file/services/secrets/) of the Compose file.

Defining a secret in the top-level `secrets` must not imply granting any service access to it.
Such grant must be explicit within service specification as [secrets](/reference/compose-file/services/secrets/) service element.
...
Note that support for `uid`, `gid`, and `mode` attributes are only implemented in Docker Compose when the source of the secret is [`environment`](/reference/compose-file/services/secrets/). When the source is a [`file`](/reference/compose-file/services/secrets/), Compose uses a bind-mount under the hood which doesn't allow `uid` remapping, and these attributes are silently ignored.
...
It's specified as a [byte value](/reference/compose-file/services/extension/#specifying-byte-values).
...
as a [duration](/reference/compose-file/services/extension/#specifying-durations).
...
For more information about the `volumes` top-level element, see [Volumes](/reference/compose-file/services/volumes/).
...
- `source`: The source of the mount, a path on the host for a bind mount, a Docker image reference for an image mount, or the
  ...
  [top-level `volumes` key](/reference/compose-file/services/volumes/). Not applicable for a tmpfs mount.
